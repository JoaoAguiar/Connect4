...
class TypesOfSearch {
  ...
  public GameBoard max_minimax(int depth, GameBoard game_board) {
    ...
    childresList = game_board.createChildren(1);
    
    GameBoard board = new GameBoard('_');

    for(GameBoard child_board : childrenList) {
      total_nodes += 1;
      int min = min_minimax(depth-1, child_board).score;

      if(minValue < min) {
        minValue = min;
        GameBoard.copyBoard(board, child_board);
      }
    }
  ...
  public GameBoard min_minimax(int depth, GameBoard game_board) {
    ...
    LinkedList<GameBoard> childrenList = new LinkedList<GameBoard>();
    childresList = game_board.createChildren(2);

    GameBoard board = new GameBoard('_');

    for(GameBoard child_board : childrenList) {
      total_nodes += 1;
      int max = max_minimax(depth-1, child_board);
  ...
  public GameBoard max_alpha_beta(int depth, int alpha, int beta, GameBoard game_board) {
    if(game_board.endGame() != 0 || depth == 0) {
      return game_board;
    }
    ...
    for(GameBoard child_board : childrenList) {
      total_nodes += 1;
      int min = min_alpha_beta(depth-1, alpha, beta, child_board);
  ...
  public int monte_carlo_tree_search(int depth, GameBoard initialGameBoard, int currentPlayer) {
    ...
    for(int i=0; i<depth; i++) {
      ...
      while(!node.isLeaf()) {
        for(Node<GameBoard> children : node.getChildrens()) {
          total_nodes += 1;
      ...
      node.addChildrens(GameBoard.possibleChildrens(node.getGameBoard(), currentPlayer));
  ...
  public int rollout(GameBoard board, int currentPlayer) {
    while(board.checkGame() == 0) {
      ...
      LinkedList<Integer> possibleActions = new LinkedList<Integer>();
        ...
        if(possibleActions.contain(random)) {
